...
{
	std::priority_queue<node> que;//优先队列

	std::vector<double> distance(Mesh.numVertices(), DBL_MAX);//最短距离 初始化为无穷大
	std::vector<int> v_p(Mesh.numVertices(), -1);//记录前驱节点的索引
	std::vector<int> is_visited(Mesh.numVertices(), 0);//记录是否访问 数组
	v_p[s_p] = s_p;
	distance[s_p] = 0;
	que.push(node(s_p, 0));
